...
def update_source(source, new_entries, source_date, download_url):
    ...
    """, (source,))
    existing = {row[0]: {
        "entity_id": row[1], "version_id": row[2],
        "program": row[3], "dob": row[4], "nationality": row[5], "name": row[6]
    ...
    if removed_ids:
        for cid in removed_ids:
            ...
            delta_rows.append((
                str(uuid.uuid4()), str(snapshot_id), existing[cid]["entity_id"],
    ...
    # ── TILLAGDA ─────────────────────────────────────────────────────────────
    # Kolla först om canonical_id redan finns (inaktiv) - återaktivera i så fall
    if added_ids:
        cur.execute("""
            SELECT canonical_id, id FROM entity
            WHERE source = %s AND canonical_id = ANY(%s) AND is_active = false
        """, (source, list(added_ids)))
        reactivate_map = {row[0]: row[1] for row in cur.fetchall()}
    else:
        reactivate_map = {}
    ...
    for cid in added_ids:
        ...
        if cid in reactivate_map:
            # Återaktivera befintlig entitet
            eid = reactivate_map[cid]
            cur.execute("UPDATE entity SET is_active = true, last_seen_at = %s, primary_name = %s WHERE id = %s",
                        (now, e.get("name", ""), eid))
        else:
            # Skapa ny entitet
            eid = str(uuid.uuid4())
            cur.execute("""
                INSERT INTO entity (id,canonical_id,source,entity_type,primary_name,first_seen_at,last_seen_at,is_active)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
                ON CONFLICT (canonical_id, source) DO UPDATE SET is_active = true, last_seen_at = EXCLUDED.last_seen_at
                RETURNING id
            """, (eid, cid, source, e.get("type", "unknown"), e.get("name", ""), now, now, True))
            row = cur.fetchone()
            if row:
                eid = str(row[0])
        ...
        delta_rows.append((
            str(uuid.uuid4()), str(snapshot_id), eid,
            "added", None, None, e.get("name", ""), now
        ))

    if added_ids:
        conn.commit()
        print(f"  Lade till/återaktiverade {len(added_ids)} entiteter")
    ...
    # Fält att jämföra: (fältnamn, nyckel i existing, nyckel i new_entry)
    WATCH_FIELDS = [
        ("name",        "name",        "name"),
    ...
    for cid in common_ids:
        ...
        # Hitta vilka fält som ändrats
        changed_fields = []
        for label, old_key, new_key in WATCH_FIELDS:
            old_val = exst.get(old_key)
            new_val = e.get(new_key)
            if old_val != new_val:
                changed_fields.append((label, old_val, new_val))
        ...
        if changed_fields:
            ...
            # Logga ett delta_log-rad per ändrat fält
            for label, old_val, new_val in changed_fields:
                delta_rows.append((
                    str(uuid.uuid4()), str(snapshot_id), exst["entity_id"],
                    "modified", label,
